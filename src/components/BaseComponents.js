...
export const Validations = {
    ...
    userTokenBalance: value => {
        ...
        return userBalance.lt(parseFloat(value)) ? "Your <nobr>A-EUR</nobr> balance is less than the amount" : undefined;
    },
    ...
    userTokenBalanceWithTransferFee: value => {
        ...
        const maxTransfer = getMaxTransfer(userBalance)
            ...
            .toString();
        if (maxTransfer <= 0) {
            return "Your <nobr>A-EUR</nobr> balance is less than the amount + transfer fee.";
        }
        return `Your <nobr>A-EUR</nobr> balance is less than the amount + transfer fee. Max amount you can transfer is ${maxTransfer} <nobr>A-EUR</nobr>`;
    },
    ...
    minOrderTokenAmount: value => {
        ...
        if (amount.gte(minValue)) {
            ...
            return `Token amount is less than minimum order amount of ${minValue} <nobr>A-EUR</nobr>`;
        }
    },
    ...
    minTokenAmount: minValue => value => {
        return parseFloat(value) < minValue ? `Amount must be at least ${minValue} <nobr>A-EUR</nobr>` : undefined;
    }
};
