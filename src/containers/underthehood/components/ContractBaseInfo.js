...
export function ContractBaseInfo(props) {
    ...
    return (
        <div>
            ...
            {connectionError ? (
                <ErrorDetails header="Connection error">{connectionError.message}</ErrorDetails>
            ) : (
                <p>No connection error</p>
            )}

            {error ? <ErrorDetails header="Tx error">{error.message}</ErrorDetails> : <p>No tx error</p>}
