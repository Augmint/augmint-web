...
library OrdersLib {
    uint80 public constant NONE = uint80(0);
    ...
    function iterateValid(OrderList storage self, uint80 _index) internal view  returns (bool) {
        return _index - 1 < self.orders.length;
    }

    function iteratePrev(OrderList storage self, uint80 _index) internal view  returns (uint80) {
        return self.orders[_index - 1].prev;
    }

    function iterateNext(OrderList storage self, uint80 _index) internal view returns (uint80) {
        return self.orders[_index - 1].next;
    }
