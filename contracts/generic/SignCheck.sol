...
contract SignCheck {
  ...
  function ecVerifyTx( address _from, address _to, uint _amount, address _signer, bytes _signature) public pure returns (bool valid, address) {
 ...
 function getEthereumKeccak256(string msgLength, string paramsString, bytes32 nonce,
    address _from, address _to, uint _amount) public pure returns (bytes32 _hash) {
    // for Remix testing
      bytes memory prefix = "\x19Ethereum Signed Message:\n";
